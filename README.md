...
코드 생성 품질과 시스템 안정성을 한 단계 더 끌어올리는 **미들웨어 아키텍처**를 도입했습니다.

### 1. 미들웨어 시스템 도입 (Middleware Architecture)

LLM 호출 과정에 **미들웨어 체인**을 삽입하여, 코드 생성의 **구조적 품질을 자동으로 강제**합니다.

*   **ValidationEnforcementMiddleware** ⭐ (핵심)
    *   생성된 코드에 `__validation_report__` 검증 변수가 **반드시 포함**되도록 보장합니다.
    *   누락 시 최대 2회까지 자동 재시도하여 검증 코드만 추가합니다. (기존 코드 보존)
    *   재시도 실패 시 `reflect` 노드로 전달하여 LLM이 맥락을 파악해 오류를 수정합니다.
*   **ForbiddenPatternMiddleware** (금지 패턴 제거)
    *   `sys.exit()`, `argparse`, `if __name__ == "__main__"` 등 **금지된 코드 패턴을 자동 감지 및 제거**합니다.
    *   샌드박스 환경에서의 비정상 종료나 인자 파싱 오류를 원천 차단합니다.
*   **OutputFormatEnforcerMiddleware** (출력 형식 정제)
    *   LLM 응답에 포함된 마크다운 펜스(` ```python `) 등을 **자동으로 제거**합니다.
    *   구문 오류를 방지하고 더 깔끔한 스크립트를 생성합니다.

### 2. 프롬프트 최적화 (토큰 절약)

미들웨어가 구조를 강제하므로, 기존 프롬프트에서 **장황한 지침(구조 강제 문구, 금지 패턴 경고 등)을 대폭 제거**했습니다.
*   **토큰 사용량 약 88% 절감** (요청당 약 850 토큰 → 약 100 토큰)
*   프롬프트가 간결해져 LLM이 본질에 더 집중합니다.

### 3. 검증 로직 설계 원칙 강화

LLM이 더 유연하고 정확한 검증 코드를 작성하도록 프롬프트에 3대 원칙을 명시했습니다.
1.  **현행화된 상태 기준 검증 (State-Awareness)**: 전처리 후 변경된 컬럼명/타입을 정확히 추적합니다.
2.  **의미적 정합성 우선 (Semantic over Literal)**: 형식보다 의미 보존을 우선합니다.
3.  **맥락적 오류 판단**: 사소한 포맷 차이로 인한 불필요한 파이프라인 중단을 방지합니다.

### 4. Validate 노드 간소화

*   미들웨어가 Report 구조를 보장하므로, `validate` 노드에서 **중복 구조 체크 로직을 제거**했습니다.
*   **내용 검증(요구사항 충족, silent miss 감지)에만 집중**하여 코드가 간결해졌습니다.

### 5. 변경 사항 요약

| 구분 | 변경 전 | 변경 후 |
|---|---|---|
| 검증 코드 생성 | 프롬프트에 의존 (실패 시 reflect) | 미들웨어가 구조 강제 (자동 재시도) |
| 금지 패턴 처리 | 프롬프트 경고 (위반 시 실행 오류) | 미들웨어가 자동 감지/제거 |
| 출력 형식 정제 | 프롬프트 지시 (가끔 실패) | 미들웨어가 자동 정제 |
| 프롬프트 토큰 | ~950 토큰/요청 | ~100 토큰/요청 |
| validate 노드 | 구조 체크 + 내용 검증 | 내용 검증만 |
